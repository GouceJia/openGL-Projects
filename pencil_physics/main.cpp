...
public:
    ...
    void drawGraphics() {
        ...
        /* TODO: Modify these draw calls to draw the bodies with the correct positions and angles. */
        // Draw red circle and white box.
        draw.circle(mat4(), redCircle.getAngle(), redCircle.getCurPos(), redCircle.radius, vec3(1,0,0));
        draw.box(mat4(), whiteBox.getAngle(), whiteBox.getCurPos(), whiteBox.size, vec3(1,1,1));
        
        // Draw all the other circles, boxes, and polylines
        for (int i = 0; i < circles.size(); i++)
            draw.circle(mat4(), circles[i].getAngle(), circles[i].getCurPos(), circles[i].radius, vec3(0,0,0));
        for (int i = 0; i < boxes.size(); i++)
            draw.box(mat4(), boxes[i].getAngle(), boxes[i].getCurPos(), boxes[i].size, vec3(0,0,0));
        for (int i = 0; i < polylines.size(); i++)
    ...
    void attachMouse(vec2 worldPoint) {
        ...
        for(int i = 0; i < boxes.size(); i++){
            if (boxes[i].contains(worldPoint)){
                ...
                def.bodyA = (b2Body*)redCircle.getBody();
                def.bodyB = (b2Body*)boxes[i].getBody();
                def.target = *new b2Vec2(worldPoint.x, worldPoint.y);
        ...
        for (int j = 0; j < circles.size(); j++){
            if (circles[j].contains(worldPoint)){
                b2MouseJointDef def;
                def = *new b2MouseJointDef();
                def.bodyA = (b2Body*)redCircle.getBody();
                def.bodyB = (b2Body*)circles[j].getBody();
                def.target = *new b2Vec2(worldPoint.x, worldPoint.y);
                def.collideConnected = true;
                def.maxForce = 100;
                def.frequencyHz = 2;
    ...
    void moveMouse(vec2 worldPoint) {
        ...
        if (mouseJoint != NULL) {
            b2Vec2 temp = b2Vec2(worldPoint.x, worldPoint.y);
            mouseJoint->SetTarget(temp);
        }
    }
