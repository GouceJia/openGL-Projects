...
public:
    ...
    void advanceState(float dt) {
        ...
        if (earth.isSpherical() != targetSpherical) {
            if (isSpherical < targetSpherical){
                isSpherical += 0.25;
            }
            else {
                isSpherical -= 0.25;
            }
            earth.setSpherical(isSpherical);
        }
    ...
    void drawGraphics() {
        ...
        for (int i = start; i <= end; i++) {
            ...
            vec3 quakePos = earth.getPosition(e.getLatitude(), e.getLongitude());
            float radius = e.getMagnitude();
            float red = 0.5 + 1*(radius-0)/7;
            glColor3f(red, 0.8, 0);
            Draw::sphere(quakePos, radius/300);
