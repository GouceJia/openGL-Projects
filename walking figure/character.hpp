...
inline void Character::draw() {
    ...
    mat4 cur_trans = getCurrentCoordinateFrame();
    vec3 cur_position = getCurrentPosition();
    
    glPushMatrix();
    
    glMultMatrixf(&cur_trans[0][0]);
...
inline void Bone::draw() {
    ...
    mat4 cur_local_trans = getCurrentLocalRotation();
    glMultMatrixf(&cur_local_trans[0][0]);
    
    // draw the current bone
    vec3 boneVec = getBoneVector();
    vec3 zAxis = vec3(0, 0, 1);
    vec3 surfaceNormal = normalize(glm::cross(boneVec, zAxis));
    float rotateAngle = acos(glm::dot(boneVec, zAxis)/(glm::length(boneVec)))*180/PI;
    
    // constructing matrix for coordinates transformation
    glPushMatrix();
        glRotatef(-rotateAngle, surfaceNormal.x, surfaceNormal.y, surfaceNormal.z);
        glScalef(0.05, 0.05, length);
        Draw::unitCylinderZ();
        glScalef(1, 1, 0.05/length);
        Draw::unitSphere();
    ...
    // recursively draw the child bones
